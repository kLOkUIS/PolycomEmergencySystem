...
static bool recoverStuckReceiveWatched() {
	// A stuck listen usually means the radio never exited the prior RX state,
	// so stop any lingering window before attempting a fresh precv().
	api.lora.precv(0);
	if (!restartReceiveWatched()) {
		return false;
	}

	// No callback has happened yet, but a successful re-arm should begin a fresh
	// stuck-watchdog observation window instead of immediately retriggering.
	gLastRadioCallbackAtMs = millis();
	return true;
}
