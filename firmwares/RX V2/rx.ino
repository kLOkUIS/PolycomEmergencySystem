...
static const KeyAction END_CALL_ACTIONS[] = {
	{KEY3_CTL_PIN, 1},
	{KEY1_CTL_PIN, 3},
};
...
static void startAction(ActionPlan plan, const KeyAction *actions, uint8_t actionCount) {
	...
	DPRINTLN(plan == ACTION_PLAN_PLACE_CALL ? "PLACE_CALL" : "END_CALL");
}
...
static void handleEndCallMessage(const ProtocolMessage &message) {
	(void)message;
	if (!callSenseActive()) {
		DPRINTLN("Ignoring END_CALL because call is not active");
		return;
	}

	if (actionBusy()) {
		DPRINTLN("Ignoring END_CALL while action plan is already running");
		return;
	}

	startAction(ACTION_PLAN_END_CALL, END_CALL_ACTIONS, sizeof(END_CALL_ACTIONS) / sizeof(END_CALL_ACTIONS[0]));
}
